...
function formatTanggalPelaksanaanPKL(dataValue) {
  if (!dataValue) {
    return "-";
  }

  const parts = String(dataValue).split(/\s*-\s*/);

  if (parts.length === 2) {
    return `${formatTanggalIndonesia(parts[0])} - ${formatTanggalIndonesia(parts[1])}`;
  }

  return formatTanggalIndonesia(dataValue);
}
...
function loadSignatureData() {
  ...
  setTextById("mahasiswaPemohon", data.m);
  const kepadaElement = document.getElementById("kepada");
  setTextById("noPenerima", data.np);
  setTextById("emailPenerima", data.ep);
  setTextById("tanggalPelaksanaan", formatTanggalPelaksanaanPKL(data.tp));
